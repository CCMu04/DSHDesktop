 ...
 *
 * 「完成提醒」功能增强的服务端：开关持久化（~/.dsh/desktop-notify.json），
 * 浏览器端通过 /api/desktop-notify/config 读写。
 */
...
/** 开关文档路径。 */
function configPath() {
  return join(homeDir(), "desktop-notify.json");
}
...
/**
 * 注册开关 API 路由：/api/desktop-notify/config（GET/HEAD/POST）。
 */
export function apply(ctx, config = {}) {
  ...
  const route = {
    kind: "exact",
    path: "/api/desktop-notify/config",
    handler: (req, res) => {
  ...
  ctx.effect(() => {
    ...
    return () => dispose();
  }, "dsh-desktop-notify: config route");
}
