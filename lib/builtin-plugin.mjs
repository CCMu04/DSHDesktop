...
/**
 * 让 builtin-plugins 里的宿主插件能解析运行时提供的 @deepseek-ai/* 包。
 *
 * 插件的物理位置在 `builtin-plugins/<name>` 下（profile 的 link 只指向这里），
 * Node 解析裸说明符沿真实路径向上走，永远够不到运行时的 node_modules——宿主半
 * 一旦 import '@deepseek-ai/*' 就会 ERR_MODULE_NOT_FOUND。把运行时的
 * @deepseek-ai 目录 junction 到 `builtin-plugins/node_modules/@deepseek-ai` 后，
 * 从任何内置插件发出的 @deepseek-ai/* 导入都能命中运行时包（语义与官方插件从
 * 运行时 node_modules 解析一致），且不随插件目录复制任何依赖。
 *
...
export function ensurePluginRuntimeExports({
  ...
  const target = path.resolve(
    path.join(runtimeNodeModulesDirectory, '@deepseek-ai'),
  )
  ...
  const link = path.join(linkDirectory, '@deepseek-ai')
